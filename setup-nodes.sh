...
if [[ ! -f "$INVENTORY" ]]; then
    ...
    echo "The RKE2 gateway host must be specified."
    exit 1
...
# Insert the ip of the RKE2 gateway host (obtained from the inventory) into the playbook file.
...
for key in "${!host_map[@]}"; do
    echo "Copying key to $key(${host_map[$key]})..."
...
EOF
    ...
    echo "Copying local repository to $key(${host_map[$key]})..."
    scp -T -o StrictHostKeyChecking=no -r local-artifacts/local-repo root@${host_map[$key]}:/tmp/ > /dev/null 2>&1
    exit_code=$?
    if [ $exit_code -ne 0 ]; then
      errors+=("Failed to copy local rpm repository to host $key")
      continue
    fi
    scp -T -o StrictHostKeyChecking=no local-artifacts/local-repo.repo root@${host_map[$key]}:/etc/yum.repos.d/local-repo.repo > /dev/null 2>&1
    exit_code=$?
    if [ $exit_code -ne 0 ]; then
      errors+=("Failed to copy local repository file to host $key")
      continue
    fi

    echo "Disabling non-local repositories on $key(${host_map[$key]})..."
    ssh -T -o StrictHostKeyChecking=no root@${host_map[$key]} << EOF
    dnf config-manager --set-disabled \*
    dnf config-manager --set-enabled local
    dnf clean all    
EOF
